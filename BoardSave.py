...
class Board(Canvas):
    def __init__(self,frame,b,size,startnode):
        ...
        self.draw=ImageDraw.Draw(self.editedimage)
        ...
        #Saves image
        self.editedimage.save("editedboard.png","PNG")
    ...
    def addEdge(self,xend,yend,col,xstart=0,ystart=0):
        ...
        self.draw.line((self.pmean(self.coords(start)),self.pmean(self.coords(destination))),fill=self.colordict[col],width=4)
                    
    def createNode(self,x,y,name=""):
        if name=="":
            p=self.create_oval(x-9,y-9,x+9,y+9,fill="black",tags="node")

            self.draw.ellipse((x-9,y-9,x+9,y+9),fill="black")
            ...
            self.draw.text((x+12,y-6),name[1:].upper(),fill="black",font=ImageFont.truetype("arial.ttf",10))

        elif name[0]=="*":
            p=self.create_rectangle(x-12,y-12,x+12,y+12,fill="black",tags="node")

            self.draw.polygon((x-12,y-12,x-12,y+12,x+12,y+12,x+12,y-12),fill="black")
            
            self.create_text(x+16,y,text=name[1:].upper(),anchor=W,font="Ariel 12 bold",tags="text")

            self.draw.text((x+16,y-8),name[1:].upper(),fill="black",font=ImageFont.truetype("arial.ttf",12))

        else:
            p=self.create_polygon(x,y-18,x-15,y+9,x+15,y+9,fill="black",
                             tags="node")

            self.draw.polygon((x,y-18,x-15,y+9,x+15,y+9),fill="black")
            
            self.create_text(x+14,y-2,text=name.upper(),anchor=W,font="Ariel 10 bold",tags="text")

            self.draw.text((x+14,y-7),name.upper(),fill="black",font=ImageFont.truetype("arial.ttf",11))
